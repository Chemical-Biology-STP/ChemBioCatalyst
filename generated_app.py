...
for menu in menu_config:
    if "pages" in menu:
        for page in menu["pages"]:
            ...
            if page.get("icon"):
                pages[var_name] = st.Page(
                    page["file_path"], title=page["display"], icon=page["icon"]
                )
            else:
                pages[var_name] = st.Page(
                    page["file_path"], title=page["display"]
                )
    else:
        ...
        if page.get("icon"):
            pages[var_name] = st.Page(
                page["file_path"], title=menu["display"], icon=page["icon"]
            )
        else:
...
with st.sidebar:
    ...
    category = st.selectbox(
        "Select a category", [m["display"] for m in menu_config]
    )
    current_menu = next(
        (m for m in menu_config if m["display"] == category), None
    )
    if current_menu:
        if "pages" in current_menu:
            option = st.selectbox(
                "Select an option",
                [p["display"] for p in current_menu["pages"]],
            )
